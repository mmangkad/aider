...
def main(
    ...
    # Configure per-run logs for error outputs and user asks (silent recording)
    # Do not create the directory here to avoid interfering with test copying logic below.
    try:
        os.environ["AIDER_BENCHMARK_ERRORS_FILE"] = str(Path(dirname) / "error_outputs.jsonl")
        os.environ["AIDER_BENCHMARK_USER_ASKS_FILE"] = str(Path(dirname) / "user_asks.jsonl")
    except Exception:
        # Do not disrupt console output or run if logging can't be configured
        pass
